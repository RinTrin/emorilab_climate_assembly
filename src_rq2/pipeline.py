...
from utils import filter_top1_score, transcribe_youtube_to_text, merge_materialtext_and_youtubetext
...
def pipeline(city_name=None):
    ...
    political_analysis(analyzed_csv_pth, presenter_role_dict, input_materials_pkl_pth_list=input_sentences_pkl)
    ghjk
    """
    情報提供者のアクターと情報提供した日にちによって、どの程度アクションプランに参照されたかを観察するための分析。
    この考えに至った背景的論文は、、、あったか？ TODO:論文まとめを読む。
    """
    actor_analysis(analyzed_csv_pth, presenter_role_dict)
    
    """
    情報提供の発表時間によって、どの程度アクションプランに参照されたかを観察するための分析。
    実験的意味合いが強い。
    """
    presentation_length_analysis(analyzed_csv_pth, presenter_role_dict)
