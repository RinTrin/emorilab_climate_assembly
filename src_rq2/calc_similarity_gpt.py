...
def select_similar_sentence(
    ...
    if not candidates:
        print(presenter_role_dict)
        print(len(input_sentences_pkl))
    ...
    for i, action_sentence in enumerate(action_sents):
        ...
        print(out)
        print(chosen)
        check_non_similar = gpt_delete_non_similar(client, model, action_sentence, chosen["sentence"])
        print(check_non_similar)
        print("=====")
        ...
        row = {
            ...
            "similar_check": check_non_similar
        }

        pd.DataFrame([row]).to_csv(
            out_csv_path,
            mode="a",
            header=not Path(out_csv_path).exists(),
            index=False,
            encoding="utf-8-sig",
        )
