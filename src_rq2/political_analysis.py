...
import torch
from utils import pickle_load, input_organize, _to_np32, _finite
...
def political_analysis(analyzed_csv_pth, presenter_role_dict, input_materials_pkl_pth_list):
    ...
    # 意見文だけを抽出
    """
    今は、意見文の抽出をするコードを記載していない。理由としては、bertをfine-tuningする必要があるからである。
    TODO: fine-tuningする。
    """
    
    opinion_vector_for_each_expert = get_opinion_vector(input_materials_pkl_pth_list)
    # print(opinion_vector_for_each_expert)
    opinion_vector_for_each_expert = {str(key).replace("_merged", ""):value for key, value in opinion_vector_for_each_expert.items()}
    # vghjk
    ...
    # --- (2) ネット記事を基準軸にして射影 ---
    # 左派：https://www.sbbit.jp/article/cont1/56791?page=2 の1page目
    # 右派：https://president.jp/articles/-/93651?page=1 の1page目
    reference_data = {
    ...
    plot_political_scores_table(df_scores, presenter_role_dict, save_dir="/Users/rintrin/codes/emorilab_climate_assembly/analysis_results/political_analysis")
    df_scores.to_csv("/Users/rintrin/codes/emorilab_climate_assembly/analysis_results/political_analysis/df_scores.csv")
    
    ghj

    # --- (3) UMAPで全体の分布を可視化 ---
    df_vis = visualize_umap_from_average_vectors_with_meta(average_vector_for_each_expert, presenter_role_dict, save_dir="/Users/rintrin/codes/emorilab_climate_assembly/analysis_results/political_analysis", reference_data=reference_data, model_name="cl-tohoku/bert-base-japanese-v3")

    vbjk
    # --- (4) BERTopicで内容トピックを抽出 ---
    # topic_model = topic_analysis_from_pickles(input_materials_pkl_pth_list)

def get_opinion_vector(input_materials_pkl_pth_list, classifier=None):
    """
    各情報提供者ごとに:
        - pickleロード
        - 意見文のみ抽出
    ...
    #     classifier = OpinionClassifier("path_to_finetuned_model")

    for input_pkl in tqdm(input_materials_pkl_pth_list, desc="Encoding opinions"):
        src_name = os.path.basename(input_pkl).replace("_segmented.pkl", "")
        data = pickle_load(input_pkl)
        sentences = input_organize(data)

        # # --- 意見文のみ抽出 ---
        # opinion_sents = [s for s in sentences if classifier.is_opinion(s)]
        opinion_sents = sentences

        if len(opinion_sents) == 0:
            print(f"⚠️ {src_name}: no opinion sentences found.")
            continue

        # --- SBERTエンコード ---
        with torch.no_grad():
            emb = model.encode(opinion_sents, batch_size=64)
            emb = F.normalize(torch.tensor(emb), p=2, dim=1)
            emb = emb.cpu().numpy()

        results[src_name] = emb
    return results
...
def plot_political_scores_table(df_scores, presenter_role_dict, save_dir):
    """
    ...
    plt.xlabel("右派 ←→ 左派 方向の射影スコア（正: 右派寄り / 負: 左派寄り）")
    ...
    # 保存
    save_path = os.path.join(save_dir, "political_projection_scores.png")
